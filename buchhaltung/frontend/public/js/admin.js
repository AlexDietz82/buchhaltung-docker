  ...
  users.forEach(user => {
    ...
    row.innerHTML = `
      ...
      <td>
        ...
        <button class="btn btn-small btn-info" onclick="editUserRights('${user.id}')">Rechte</button>
...
async function saveUser() {
  try {
    // Standardrechte beim Anlegen: alle Seiten, keine Konten
    const data = {
...
function editUserRights(userId) {
  ...
  // Seiten-Checkboxen rendern
  const pagesDiv = document.getElementById('pages-checkboxes');
  pagesDiv.innerHTML = '';
  allPages.forEach(page => {
    const checked = Array.isArray(user.pages) && user.pages.includes(page.key) ? 'checked' : '';
    pagesDiv.innerHTML += `
      <label>
  ...
  // Konten-Checkboxen rendern
  const accountsDiv = document.getElementById('accounts-checkboxes');
  accountsDiv.innerHTML = '';
  accounts.forEach(account => {
    // accounts können als String oder als Number gespeichert sein
    const accountIdStr = String(account.id);
    const userIds = Array.isArray(user.accounts) ? user.accounts.map(String) : [];
    const checked = userIds.includes(accountIdStr) ? 'checked' : '';
    accountsDiv.innerHTML += `
      <label>
        <input type="checkbox" name="accounts" value="${account.id}" ${checked}>
        ${account.name}
      </label><br>
    `;
  });

  document.getElementById('rights-modal-title').innerText = `Rechte für ${user.username} bearbeiten`;
  document.getElementById('rights-modal-overlay').classList.remove('hidden');
}

function closeRightsModal() {
  document.getElementById('rights-modal-overlay').classList.add('hidden');
  currentRightsUserId = null;
}
...
async function saveUserRights() {
  try {
    ...
    const selectedAccounts = Array.from(form.querySelectorAll('input[name="accounts"]:checked')).map(cb => cb.value);

    // Nutze den bisherigen User (damit role, username etc. nicht verloren gehen)
    const user = users.find(u => u.id === currentRightsUserId);
    if (!user) throw new Error('Benutzer nicht gefunden');

    // Erstelle updated-Objekt
    const data = {
    ...
    // Passwort-Feld nicht mitschicken
    await apiRequest(`/users/${currentRightsUserId}`, { method: 'PUT', body: JSON.stringify(data) });

    showSuccess('Rechte gespeichert');
    closeRightsModal();
    loadUsers();
  } catch (error) {
    showError(error.message || 'Fehler beim Speichern der Rechte');
  }
}
